...
/* MARK NAME Alícia Marzola Chaves */
/* MARK NAME José Gabriel Vieira de Souza */
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
     ...
     * RESPOSTA: ###############################################*
     * Verifica que o processo a ser executado é o 'cd' e que*
     * o comando é de exec() ' ', portanto será acessado um *
     * diretório filho da pasta atual. Como o chdir() *
     * afeta apenas o processo que o executa, *
     * ele deve ser tratado diretamente no shell. *
     * #########################################################*/
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "cd: erro ao mudar de diretório\n");
      continue;
    }
    /* MARK END task1 */
